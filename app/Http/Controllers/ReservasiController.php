...
class ReservasiController extends Controller {
 ...
 private function validated(Request $r): array { $rules=['lapangan_id'=>['required','exists:lapangans,id'],'tanggal'=>['required','date','after_or_equal:today'],'jam_mulai'=>['required','date_format:H:i'],'jam_selesai'=>['required','date_format:H:i','after:jam_mulai'],'catatan'=>['nullable','max:500']]; if(auth()->user()->role==='admin')$rules['pelanggan_id']=['required','exists:pelanggans,id']; return $r->validate($rules); }
 private function save(array $data,?Reservasi $item=null): void { DB::transaction(function()use($data,$item){$pelangganId=auth()->user()->role==='admin'?$data['pelanggan_id']:auth()->user()->pelanggan?->id; abort_unless($pelangganId,422,'Profil pelanggan belum tersedia.'); $lapangan=Lapangan::whereKey($data['lapangan_id'])->where('status','tersedia')->lockForUpdate()->firstOrFail(); $bentrok=Reservasi::where('lapangan_id',$data['lapangan_id'])->whereDate('tanggal',$data['tanggal'])->when($item,fn($q)=>$q->whereKeyNot($item->id))->whereNotIn('status_reservasi',['dibatalkan'])->where('jam_mulai','<',$data['jam_selesai'])->where('jam_selesai','>',$data['jam_mulai'])->lockForUpdate()->exists(); if($bentrok)throw ValidationException::withMessages(['jam_mulai'=>'Jadwal bentrok. Lapangan sudah dipesan pada rentang waktu tersebut.']); $menit=Carbon::createFromFormat('H:i',$data['jam_mulai'])->diffInMinutes(Carbon::createFromFormat('H:i',$data['jam_selesai'])); $durasi=(int)ceil($menit/60); $payload=['pelanggan_id'=>$pelangganId,'lapangan_id'=>$data['lapangan_id'],'tanggal'=>$data['tanggal'],'jam_mulai'=>$data['jam_mulai'],'jam_selesai'=>$data['jam_selesai'],'durasi'=>$durasi,'harga_per_jam'=>$lapangan->harga_per_jam,'total_harga'=>$durasi*$lapangan->harga_per_jam,'catatan'=>$data['catatan']??null]; if(!$item){$payload+=['kode_reservasi'=>'RSV-'.now()->format('Ymd').'-'.strtoupper(str()->random(6)),'status_reservasi'=>'menunggu','status_pembayaran'=>'belum_bayar']; Reservasi::create($payload);}else{$item->update($payload);} },3); }
