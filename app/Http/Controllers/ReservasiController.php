...
class ReservasiController extends Controller {
 ...
 public function create(Request $r): View {
  $r->validate(['lapangan_id'=>['nullable','exists:lapangan,id'],'tanggal'=>['nullable','date','after_or_equal:today'],'jam_mulai'=>['nullable','date_format:H:i']]);
  $item=new Reservasi;
 ...
 private function validated(Request $r): array { $rules=['lapangan_id'=>['required','exists:lapangan,id'],'tanggal'=>['required','date','after_or_equal:today'],'jam_mulai'=>['required','date_format:H:i'],'jam_selesai'=>['required','date_format:H:i','after:jam_mulai'],'dp_amount'=>['required','numeric','min:50000'],'catatan'=>['nullable','max:500']]; if(auth()->user()->role==='admin'){$rules['pelanggan_id']=['required','exists:pelanggan,id'];$rules['status_reservasi']=['required',Rule::in(['menunggu','dikonfirmasi','selesai','dibatalkan'])];$rules['status_pembayaran']=['required',Rule::in(['sebagian','lunas'])];} return $r->validate($rules); }
